...
*/
int HashTable::computeHash(const string &arraySize) {
    int hash = 0;

    for (unsigned i = 0; i < arraySize.size(); ++i) {
        hash += static_cast<int>(arraySize.at(i));
...
*/
void HashTable::put(const string &userWord, int score) {
	if(contains(userWord)){
        int ind = computeHash(userWord);
        for(auto i = hashTable[ind].begin(); i != hashTable[ind].end(); i++){
            if(i->getWord() == userWord){
        ...
        int ind = computeHash(userWord);
        WordEntry* word = new WordEntry(userWord, score);
        hashTable[ind].push_back(*word);
    }
}
